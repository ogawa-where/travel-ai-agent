...
import type { User, TravelPlan, TravelChatResponse } from '../lib/api'
...
const initializeChat = async () => {
  ...
  try {
    ...
    const response = await api.startTravelChat(props.user.id)
    ...
    console.error('Failed to initialize travel planning chat:', error)
    messages.value = [{
      role: 'assistant',
...
<template>
  <div class="travel-planning-view">
    <div class="view-header">
      <h2>旅行企画モード</h2>
      <p>行き先や日程を教えてください。あなたに合ったプランを作成します。</p>
    </div>
