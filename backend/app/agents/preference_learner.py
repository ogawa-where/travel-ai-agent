...
3. 旅行の傾向（予算より体験重視、計画より即興、など）を把握する（tendency）

注意:
- 「予算3万円」「2泊3日」などの具体的な制約は扱わない（それは旅行企画モードで扱う）
- ここでは「予算を重視するか/体験を重視するか」などの傾向を学習する
...
- tendency: 旅行の傾向（予算より体験重視、計画より即興、ゆっくり派 など）

注意:
- 「予算3万円」「2泊3日」などの具体的な数値制約は抽出しない（旅行企画モードで扱う）
- 「予算を重視する」「体験を重視する」などの傾向は tendency として抽出する
...
{{
  "signals": [
    {{
      ...
      "tag": "嗜好のタグ（例: 文化体験, 自然, アウトドア, 体験重視）",
